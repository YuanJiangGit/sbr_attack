...
from htps.HTPsAttackV1 import InsertHtps
...
from htps.search.Search2 import Search2


class HTPsAttackV3(AttackRecipe):
    nsbr_top_words = []
    top_words = []
    ...
    def __init__(self, project, model_type):
        ...
        with open(nsbr_top_words_path, 'r+') as f:
            HTPsAttackV3.nsbr_top_words = f.read().split()
        with open(top_words_path, 'r+') as f:
            HTPsAttackV3.top_words = f.read().split()
    ...
    def build(model):
        transformation = CompositeTransformationV1([
            ...
            # WordSwapWordNetV1(),
            InsertHtps(top_words=HTPsAttackV3.nsbr_top_words),
        ])
        stopwords = set(
        ...
        search_method = Search2(beam_width=8)
