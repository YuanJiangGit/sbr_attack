...
class ConstraintV1(ABC):
    ...
    def call_many(self, transformed_texts, reference_text, **kwargs):
        ...
        filtered_texts = self._check_constraint_many(
            compatible_transformed_texts, reference_text, **kwargs
        )
        return list(filtered_texts) + incompatible_transformed_texts

    def _check_constraint_many(self, transformed_texts, reference_text, **kwargs):
        """Filters ``transformed_texts`` based on which transformations fulfill
        ...
        return [
            transformed_text
            for transformed_text in transformed_texts
            if self._check_constraint(transformed_text, reference_text, **kwargs)
        ]
