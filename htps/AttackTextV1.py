...
class AttackedTextV1:
    ...
    def __init__(self, text_input, attack_attrs=None):
        ...
        self.position_reflect = dict([(i, i) for i in range(len(self.words))])
        # 记录当前候选样本上一次执行的扰动策略，0代表插入，1代表替换
        self.perturbed_weight = {'insert': 0.4, 'swap': 0.6}
        self.sorted_score = 0
        self.attack_attrs.setdefault("perturbed_num", 0)
        self.attack_attrs.setdefault("perturb_method", -1)
        self.attack_attrs.setdefault("insert_num", 0)
        self.attack_attrs.setdefault("swap_num", 0)
        self.attack_attrs.setdefault("ppl_score", 0)
    ...
    def all_words_diff(self, other_attacked_text):
        ...
        # indices = set()
        # new_words = set(self.words)
        # orig_words = set(other_attacked_text.words)
        ...
        return self.attack_attrs["perturbed_num"]
    ...
    def generate_new_attacked_text(self, new_words):
        ...
        """
        perturbed_text = ""
        original_text = AttackedTextV1.SPLIT_TOKEN.join(self._text_input.values())
        ...
        perturbed_input_texts = perturbed_text.split(AttackedTextV1.SPLIT_TOKEN)
        perturbed_input = OrderedDict(
            zip(self._text_input.keys(), perturbed_input_texts)
        )
        return AttackedTextV1(perturbed_input, attack_attrs=new_attack_attrs)
