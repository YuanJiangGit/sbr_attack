...
from htps.search.Search3 import Search3


class HTPsAttackV4(AttackRecipe):
    nsbr_top_words = []
    top_words = []
    ...
    def __init__(self, project, model_type):
        ...
        with open(nsbr_top_words_path, 'r+') as f:
            HTPsAttackV4.nsbr_top_words = f.read().split()
        with open(top_words_path, 'r+') as f:
            HTPsAttackV4.top_words = f.read().split()
    ...
    def build(model):
        transformation = CompositeTransformationV1([
            ...
            # WordSwapWordNetV1(),
            InsertHtps(top_words=HTPsAttackV4.nsbr_top_words),
        ])
        stopwords = set(
        ...
        search_method = Search3(beam_width=8)
