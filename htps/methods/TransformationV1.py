...
class TransformationV1(ABC):
    ...
    def __call__(
        ...
        # 想要修改的原始位置索引信息
        original_indices_to_modify = list(indices_to_modify.copy())
        # 将原始位置信息进行再映射变成当前样本的修改的位置信息
        indices_to_modify = set([current_text.position_reflect[one] for one in indices_to_modify])

        for constraint in pre_transformation_constraints:
            indices_to_modify = indices_to_modify & constraint(current_text, self)
        transformed_texts = self._get_transformations(current_text, original_text, indices_to_modify, original_indices_to_modify)
