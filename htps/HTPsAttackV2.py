...
from htps.search.Search1 import Search1


class HTPsAttackV2(AttackRecipe):
    nsbr_top_words = []
    top_words = []
    ...
    def __init__(self, project, model_type):
        ...
        with open(nsbr_top_words_path, 'r+') as f:
            HTPsAttackV2.nsbr_top_words = f.read().split()
        with open(top_words_path, 'r+') as f:
            HTPsAttackV2.top_words = f.read().split()
    ...
    def build(model):
        transformation = CompositeTransformationV1([
            ...
            # WordSwapWordNetV1(),
            InsertHtps(top_words=HTPsAttackV2.nsbr_top_words),
        ])
        stopwords = set(
        ...
        search_method = Search1(beam_width=8)
